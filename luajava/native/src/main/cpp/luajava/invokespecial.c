...
jvalue convertFromJobject(JNIEnv *env, jobject obj, char target) {
    ...
    switch (target) {
        case 'V': // Void
            ...
            break;
        case 'Z': // Boolean
            value.z = (*env)->CallBooleanMethod(env, obj, boolean_boolvalue);
            break;
        case 'C': // Character
            value.c = (*env)->CallCharMethod(env, obj, character_charvalue);
            break;
        case 'B': // Number: Byte
            value.b = (*env)->CallByteMethod(env, obj, number_bytevalue);
            break;
        case 'D': // Number: Double
            value.d = (*env)->CallDoubleMethod(env, obj, number_doublevalue);
            break;
        case 'F': // Number: Float
            value.f = (*env)->CallFloatMethod(env, obj, number_floatvalue);
            break;
        case 'I': // Number: Integer
            value.i = (*env)->CallIntMethod(env, obj, number_intvalue);
            break;
        case 'J': // Number: Long
            value.j = (*env)->CallLongMethod(env, obj, number_longvalue);
            break;
        case 'S': // Number: Short
            value.s = (*env)->CallShortMethod(env, obj, number_shortvalue);
            break;
        default:
            value.l = obj;
            break;
...
#define CALL_NONVIRTUAL(TYPE, FIELD, CLAZZ, CONSTRUCTOR)                     \
    v.FIELD = (*env)->CallNonvirtual##TYPE##MethodA(env, obj, clazz, method, args); \
    RETURN_IF_ERROR(env, L);                                                 \
    v.l = (*env)->NewObject(env, CLAZZ, CONSTRUCTOR, v.FIELD);               \
